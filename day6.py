key = '''turn on 887,9 through 959,629
...
def day6part1(key: str) -> int:
    dimensions = 1000
    grid = [[0 for i in range(dimensions)] for i in range(dimensions)]

    for direction in key.splitlines():
        direction = direction.split(" ")

        startX, startY = [int(i) for i in direction[-3].split(",")]
        endX, endY = [int(i) for i in direction[-1].split(",")]

        for y in range(startY, endY + 1):
            for x in range(startX, endX + 1):
                if direction[0] == "turn":
                    grid[y][x] = 0 if direction[1] == "off" else 1
                else:
